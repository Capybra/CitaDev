...
// API: Обновление через Git (Исправлено)
if (strpos($uri, '/api/update') !== false) {
    header('Content-Type: application/json');
    if (!file_exists($gitPath)) {
        echo json_encode(['status' => 'error', 'message' => 'Git не найден: ' . $gitPath]);
        exit;
    }
    // Команда с принудительным переходом в папку проекта
    $command = "cd /d \"$baseDir\" && \"$gitPath\" pull 2>&1";
    $output = shell_exec($command);
    echo json_encode([
        'status' => 'success', 
        'message' => 'Команда Git выполнена', 
        'output' => $output ?: 'Нет вывода (возможно, уже обновлено)'
    ]);
    exit;
}
